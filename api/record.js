...
export default function(server, db) {
  ...
  server.get(`/task/:taskid/record/:bizid`, async (req, res, next) => {
    const result = await recordDB.findOne({ _task: req.params['taskid'] || '', _biz: req.params['bizid'] || '' });
    res.send(result);
  ...
  server.post(`/record`, async (req, res, next) => {
    ...
    await taskDB.findOneAndUpdate({ _id: postRecordInfo._task }, {
      $push: {
    ...
    if (task.progress[0] >= task.progress[1]) {
      ...
      if (tasks.every(t => t.state === 3)) {
        await planDB.findOneAndUpdate({ _id: task._plan }, {
          $set: {
            state: 4
          }
        });
